...
JNIEXPORT jintArray JNICALL Java_twisk_simulation_Simulation_start_1simulation (JNIEnv *env, jobject obj, jint nbEtapes, jint nbServices, jint nbClients, jintArray tabJetonsService)
{
  // récupération du tableau d'entiers passé en argument de la fonction
  jint *tab = (*env)->GetIntArrayElements(env, tabJetonsService, 0);

  int* tabPid = start_simulation(nbEtapes, nbServices, nbClients, tab) ;
